...
\begin{itemize}
  ...
  gate-expression IF, such as \code{IF (AND -I A B -O B) = 1}, has no bit on
  \textbf{c}: its yellow line spans the wires it reads, with a yellow tap on
  each, and its label shows the expression (\texttt{AND(A,B)} above
  \texttt{= 1}). The wrapper dialog shows such a test read-only; edit it in
  the protocol text.
