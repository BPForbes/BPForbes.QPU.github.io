...
IF / ELSE wrapper & Makes the gate conditional on a measured bit.\\
Kind & \textbf{IF} or \textbf{ELSE}. Choosing ELSE sets \textbf{Equals} to
0; choosing IF sets it to 1. Either can then be changed.\\
Classical bit & Which measured wire the condition reads, shown as
\textbf{q$n$ $\rightarrow$ c}. That wire needs a MEASURE (\textbf{M}) before
this gate.\\
Equals & The value that makes the gate run: 1 or 0.\\
...
bit does not match. It is written to the protocol editor as a \code{-IF}
condition, such as \code{Z -I $Q2:0 -O $Q2:0 -IF Q0=0}. The \textbf{REC} wrapper
only labels the gate with a depth badge. The gate still runs once, and the tag
is not written to the protocol text. To repeat gates DEPTH times, write a child
process with \code{REC} or \code{TREC} and call it with
\code{RUNCHILD} \ldots{} \code{-DEPTH} $n$; see bounded recursion in the Language
Reference.
\end{cautionbox}
...
\begin{itemize}
  ...
  wrapper dialog shows such a test the same way, read-only; edit it in the
  protocol text.
