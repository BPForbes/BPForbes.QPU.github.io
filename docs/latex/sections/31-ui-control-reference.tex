...
\begin{itemize}
  ...
  removes that measurement step. Later gates on the same qubit stay.
  \item \textbf{Line styles:} every qubit wire is a single line. \textbf{c}
  is the only double line. A double stroke, not an arrow, drops from the
  meter to \textbf{c} at that column. The number beside the stroke is the
  bit whose reading happened at that time.
  \item \textbf{Outlines:} the step that just ran is red on a forward gate.
  An inverse gate is blue. The active inverse step is purple.
