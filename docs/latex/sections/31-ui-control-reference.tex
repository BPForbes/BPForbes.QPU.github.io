...
its start state. Measured qubits add classical rows \textbf{c0}, \textbf{c1},
\ldots{} underneath, one per measured qubit. A circuit with no measurement
has none. Time runs left to right: every gate occupies its own column.
\begin{itemize}
  ...
  workbench to choose them yourself. Classical rows cannot be targeted.
  \item \textbf{Removing a gate:} click the gate symbol. A measurement sits on
  the classical row for that qubit.
  \item \textbf{Line styles:} every qubit wire is a single line. Classical
  rows are the only double lines. Each measurement drops from its qubit onto
  its own classical row. A red outline marks the step that just ran.
\end{itemize}
