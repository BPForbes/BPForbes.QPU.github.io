...
Its inverse is $T^\dagger=P(-\pi/4)$. \code{BT} lowers to that gate, and
\code{BS} lowers to $S^\dagger=P(-\pi/2)$. \code{BPHASE=pi/4} remains the
spelling for the same negative phase.
\begin{lstlisting}
T -I Q -O Q
\end{lstlisting}
\begin{center}
...
source.) \code{BPHASE=angle} negates the phase angle. \code{BS} and \code{BT}
emit the negative phase gates above. For every other primitive the prefix is
metadata, and emitting the ordinary gate is already the inverse for these
self-inverse gates:
\[
X,\ Y,\ Z,\ H,\ \mathrm{CNOT},\ \mathrm{CCNOT},\
\mathrm{CZ},\ \mathrm{CY},\ \mathrm{SWAP}.
\]
\code{BS} and \code{BT} synthesize $S^\dagger$ and $T^\dagger$. A \code{B}
prefix on a derived gate such as \code{BAND} is still an unknown opcode.
