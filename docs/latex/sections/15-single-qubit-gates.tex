...
X, Y, Z, H, S, T, and PHASE act on one wire, so the workbench uses only
\textbf{Target particle}; \textbf{Control A} and \textbf{Control B} are
ignored. PHASE also shows a \textbf{Phase angle} slider. On the canvas, drop
...
A primitive gate accepts \code{dg} (dagger) or \code{inv} (inverse), written
...
A \code{dg} or \code{inv} marker on a derived gate such as \code{ANDdg} is an
unknown opcode. \code{MEASUREdg} parses and warns, because measurement has no
inverse. Compiler-internal \code{RESET} remains unavailable in source.
