...
\begin{beginnerbox}[Placing a single-qubit gate]
X, Y, Z, H, S, T, RX, RY, RZ, and PHASE act on one wire, so the workbench uses only
\textbf{Target particle}; \textbf{Control A} and \textbf{Control B} are
ignored. PHASE, RX, RY, RZ, and CPHASE also show a \textbf{Phase angle} slider. On the canvas, drop
the block on the wire you want to change.
\end{beginnerbox}
...
Its inverse is $T^\dagger=P(-\pi/4)$. \code{Tdg} and \code{Tinv} lower to that
gate, and \code{Sdg} and \code{Sinv} lower to $S^\dagger=P(-\pi/2)$.
\code{PHASEdg=pi/4} is the same negative phase.
\begin{lstlisting}
T -I Q -O Q
\end{lstlisting}
\begin{center}
...
PHASEdg=pi/4 -I Q -O Q    # P(-pi/4)
invPHASE=pi/2 -I Q -O Q    # P(-pi/2)
\end{lstlisting}
...
\GateHeading{X rotation}{RX}{\GateSegment{$R_x(\theta)$}}
...
\end{bmatrix}.
\]
RX rotates about the Bloch $x$-axis. \code{RX=pi} is X up to global phase.
\begin{lstlisting}
RX=pi/2 -I Q -O Q
RX=90d -I Q -O Q
RXdg=pi/4 -I Q -O Q
...
\GateHeading{Y rotation}{RY}{\GateSegment{$R_y(\theta)$}}
...
\end{bmatrix}.
\]
RY rotates about $y$ and is useful for preparing real superpositions from $\ket0$.
\begin{lstlisting}
RY=pi/2 -I Q -O Q
\end{lstlisting}

\GateHeading{Z rotation}{RZ}{\GateSegment{$R_z(\theta)$}}
...
\end{bmatrix}.
\]
RZ rotates about $z$. It differs from PHASE by a global phase that does not
affect single-qubit measurement probabilities.
\begin{lstlisting}
RZ=pi/4 -I Q -O Q
\end{lstlisting}

\subsection{Dagger and inverse syntax}

A reversible gate accepts \code{dg} (dagger) or \code{inv} (inverse), written
after the opcode or before it. Both spellings set \code{reverse=true}.
Self-inverse gates such as X, CNOT, AND, and OR still accept the marker so
intent is visible, even though the matrix is unchanged.
\begin{lstlisting}
Xdg -I Q -O Q
invH -I Q -O Q
CNOTdg -I A -O T
...
PHASEdg=pi/4 -I Q -O Q     # P(-pi/4)
\end{lstlisting}
\code{Sdg}, \code{Sinv}, \code{dgS}, and \code{invS} synthesize $S^\dagger$.
\code{Tdg}, \code{Tinv}, \code{dgT}, and \code{invT} synthesize $T^\dagger$.
\code{PHASEdg=angle}, \code{PHASEinv=angle}, \code{dgPHASE=angle}, and
\code{invPHASE=angle} negate the angle. For these self-inverse gates the
marker emits the ordinary gate, which is already its own inverse:
\[
X,\ Y,\ Z,\ H,\ \mathrm{CNOT},\ \mathrm{CCNOT},\
\mathrm{CZ},\ \mathrm{CY},\ \mathrm{SWAP}.
\]
On the canvas the dagger prints $\dagger$ and uses a blue outline. While that
step is active the outline is purple. A forward gate uses a red outline only
while its step is active. The palette \textbf{Inverse} toggle drops the dagger
instead of the forward gate.

A \code{dg} or \code{inv} marker on a reversible derived gate such as
\code{ANDdg} is accepted and is self-inverse. \code{MEASUREdg} parses and warns,
because measurement has no inverse. Compiler-internal \code{RESET} remains
