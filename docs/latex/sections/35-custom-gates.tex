...
\emph{Requirements for a reversible gate}, in the Theory Guide's mathematical
foundations chapter, states the four general conditions --- unitarity,
bijective mapping, equal inputs and outputs, and isolation from measurement
--- that this section applies to custom gates specifically.
...
running the dagger returns every input exactly (up to one global phase shared by
all inputs), distinct inputs give distinct outputs,
and every output has norm 1. A block may use another custom gate only if that
gate passed the same check; replacing or removing a gate re-checks the blocks
that use it. If the block fails, the error names the reason.
\end{cautionbox}
