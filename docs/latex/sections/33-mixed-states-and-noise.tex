...
For two qubits, or a qubit and a qutrit, the test is exact.
...
the X basis: what a measurement reveals depends on the observable. QPU source
does not yet have syntax for choosing a basis, so written \code{MEASURE}
commands are always Z measurements.
...
($4^n$ entries). Noisy simulation is available in the physics engine but is
not yet exposed in the Circuit builder.
\end{cautionbox}
