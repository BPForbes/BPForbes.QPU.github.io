\section{State, token, and logical-cycle commands}
...
\subsection{\code{INCREASECYCLE} --- logical cycle boundary (\implemented)}

\begin{lstlisting}
INCREASECYCLE
\end{lstlisting}
The compiler flushes pending zero-initializations into an internal batched
\code{RESET}, increments the logical cycle counter, and prepares workspace for
following operations. Cycle suffixes on tokens do not create separate visual
wires.

Throughout QPU, ``cycle'' always means a \emph{logical cycle}: a stage of the
program. Logical cycles organize protocol stages. They are not wall-clock
timing guarantees, not physical elapsed time, and do not cause gates to
execute concurrently. Noise and decoherence never advance with logical cycles;
they use a separate per-gate physical duration (Theory Guide: \emph{Mixed
states, entanglement measures, and noise}). The final circuit is still an
ordered gate sequence. Each emitted gate records the timeline cycle, and the
canvas draws a dashed vertical line across every wire at
\code{INCREASECYCLE}, labelled \textbf{IC} with the new cycle number
(\texttt{IC1}, \texttt{IC2}, \ldots). The line turns red while that step is
active. Inside a collapsed recursive call these markers are not drawn; the
teal depth badge replaces them. An integer suffix such as
...
\subsection{Logical-cycle state evolution}
...
The Interactive Workbench button \textbf{Add logical cycle boundary} inserts the same
