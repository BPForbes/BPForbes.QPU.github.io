...
Each particle card instead draws its wire's own reduced state: the coordinates
are the measurable averages $x=\langle X\rangle$, $y=\langle Y\rangle$,
$z=\langle Z\rangle$, and a wire entangled with others points to the inside
of the sphere ($r<1$). The chapter \hyperlink{mixed-states}{Mixed states,
entanglement measures, and noise} explains why.
