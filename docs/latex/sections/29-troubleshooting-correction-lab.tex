...
\subsection{Warnings for accepted but inactive syntax}

JOIN, SPLIT, FREE, DELETETOKEN, COMPILEPROCESS, MASTERVAL,
\code{SAVE_STATE}, and \code{LOAD_STATE} are accepted for compatibility but do not currently
change the simulated circuit. The \code{-$R} flag is also preserved without
changing substitution. A warning asks the author to confirm that metadata-only
behavior is intentional. \code{DECLARECHILD} is not in that list: it binds a
catalog child so later \code{RUNCHILD}/\code{CALL} rows can expand it.
...
It cannot make accepted-but-unimplemented operations execute, and it must not
rewrite canonical bundled truth-table expectations.
...
ACCEPTED\_ONLY\_OPERATION & The instruction currently emits no simulator
behavior. & Replace it with implemented behavior or accept metadata-only use.\\
INACTIVE\_PARAMETER\_FLAG & \code{-$R} has no current execution effect. & Use
explicit bindings.\\
INACTIVE\_REVERSE\_PREFIX & A B-prefixed gate is not being inverted. & Use a
self-inverse gate or BPHASE where appropriate.\\
