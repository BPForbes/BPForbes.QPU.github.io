/**
 * Frozen pre-PhysicsEngine state-vector simulator (commit 23e64ef), kept only
 * as a regression oracle. The kernels below are copied verbatim from the old
 * gates/operations.ts; runLegacyCircuit reproduces the old per-gate dispatch.
 * Do not use outside tests and do not "fix" it: its job is to stay old.
 */
import { add, type Complex, complex, magnitudeSquared, mul, ONE, scale, ZERO } from '../../complex';
import {
  MATRIX_H,
  MATRIX_X,
  MATRIX_Y,
...
import type { CircuitGate, MeasurementMap, ParticleStartState } from '../../types';
...
// ── Old gate dispatch (preconfigured gates only) ─────────────────────────

const legacyGate = (state: Complex[], n: number, gate: CircuitGate, random: () => number, measurements: MeasurementMap): Complex[] => {
  const target = gate.targets[0];
  const angle = gate.phase ?? 0;
  if (gate.inverse && (gate.type === 'S' || gate.type === 'T')) {
    return applySingleQubitGate(state, n, target, phaseMatrix(gate.type === 'S' ? -Math.PI / 2 : -Math.PI / 4));
  }
  switch (gate.type) {
    case 'X':
    case 'NOT':
      return applySingleQubitGate(state, n, target, MATRIX_X);
    case 'Y':
      return applySingleQubitGate(state, n, target, MATRIX_Y);
    case 'H':
      return applySingleQubitGate(state, n, target, MATRIX_H);
    case 'Z':
      return applySingleQubitGate(state, n, target, phaseMatrix(PHASE_PI));
    case 'S':
      return applySingleQubitGate(state, n, target, phaseMatrix(PHASE_PI_2));
    case 'T':
      return applySingleQubitGate(state, n, target, phaseMatrix(PHASE_PI_4));
    case 'PHASE':
      return applySingleQubitGate(state, n, target, phaseMatrix(angle));
    case 'RX':
      return applySingleQubitGate(state, n, target, rotationXMatrix(angle));
    case 'RY':
      return applySingleQubitGate(state, n, target, rotationYMatrix(angle));
    case 'RZ':
      return applySingleQubitGate(state, n, target, rotationZMatrix(angle));
    case 'CNOT':
    case 'CCNOT':
    case 'AND':
      return applyControlledX(state, n, gate.controls, target);
    case 'NAND':
      return applySingleQubitGate(applyControlledX(state, n, gate.controls, target), n, target, MATRIX_X);
    case 'OR':
      return applyControlledPredicateX(state, n, gate.controls, target, anyControlIsActive);
    case 'XOR':
      return applyControlledPredicateX(state, n, gate.controls, target, controlsHaveParity);
    case 'CZ':
      return applyControlledZ(state, n, gate.controls, target);
    case 'CY':
      return applyControlledSingleQubit(state, n, gate.controls, target, MATRIX_Y);
    case 'CPHASE':
      return applyControlledPhase(state, n, gate.controls[0], target, angle);
    case 'SWAP':
      return applySwap(state, n, gate.targets[0], gate.targets[1]);
    case 'MEASURE': {
      const measured = measureQubit(state, n, target, random());
      measurements[target] = measured.value;
      return measured.state;
    }
    default:
      throw new Error(`Legacy oracle does not model ${gate.type}.`);
  }
};

/** Old createInitialState + runCircuit for a fixed-width register of preconfigured gates. */
export const runLegacyCircuit = (
  qubitCount: number,
  gates: CircuitGate[],
  startStates: ParticleStartState[] = [],
  random: () => number = Math.random,
): { state: Complex[]; measurements: MeasurementMap } => {
  let state = Array.from({ length: 2 ** qubitCount }, () => ZERO);
  state[0] = ONE;
  startStates.slice(0, qubitCount).forEach((start, qubit) => {
    if (start === '1p' || start === 'sp') state = applyStartState(state, qubitCount, qubit, start);
  });
  ...
  gates
    ...
    .forEach((gate) => {
      if (gate.condition && measurements[gate.condition.qubit] !== gate.condition.equals) return;
      state = legacyGate(state, qubitCount, gate, random, measurements);
    });
