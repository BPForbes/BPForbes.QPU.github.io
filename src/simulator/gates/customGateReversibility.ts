...
import { basisProbabilities, innerProduct, marginalProbabilities, resolveStateQubitCount, truncateStateVector } from '../physics/state/StateVector';
...
const conjugateDot = (left: Complex[], right: Complex[]): Complex => innerProduct(left, right);
...
const trimNestedWorkspace = (expanded: Complex[], qubitCount: number) =>
  truncateStateVector(expanded, resolveStateQubitCount(expanded, qubitCount), qubitCount);
...
const leavesWorkspaceDirty = (output: Complex[], { workspace, qubitCount }: WireLayout) =>
  workspace.length > 0 && marginalProbabilities(output, qubitCount, workspace).some((probability, index) => index > 0 && probability > EPSILON);
...
const recoveryIssue = (layout: WireLayout, bits: number, output: Complex[], recovered: Complex[], globalPhase: Complex) => {
  ...
  if (basisProbabilities(recovered)[index] < 1 - EPSILON) {
